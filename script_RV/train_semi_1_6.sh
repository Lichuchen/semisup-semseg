...
srun \
  --mpi=pmi2  --gres=gpu:${GPU_NUM}  -n$1 --ntasks-per-node=${GPU_NUM} --cpus-per-task=2 --partition=VI_OP_1080TI --job-name=7Net \
python -u ../train_s4GAN.py  \
--dataset RV  \
--checkpoint-dir ../checkpoints/RV2/RV_semi_1_6 \
--labeled-ratio 0.1667 \
--ignore-label 255 \
--num-classes 2 \
...
--save-pred-every 500 \
--num-steps 12000 \
2>&1 | tee ../log/RV/traino_semi_gan_1_6.log
