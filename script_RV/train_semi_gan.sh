...
srun \
  --mpi=pmi2  --gres=gpu:${GPU_NUM}  -n$1 --ntasks-per-node=${GPU_NUM} --cpus-per-task=2 --partition=VI_OP_1080TI --job-name=3Net \
python -u ../train_s4GAN.py  \
--dataset RV  \
--checkpoint-dir ../checkpoints/RV2/RV_semi_0_125 \
--labeled-ratio 0.125 \
--ignore-label 255 \
--num-classes 2 \
...
--save-pred-every 500 \
--num-steps 15000 \
2>&1 | tee ../log/RV/traino_semi_gan_1_8.log
