...
class ISICDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, 'test',"ISIC-2017_Validation_Data/%s.jpg" % name)
            label_file = osp.join(self.root,'test', "ISIC-2017_Validation_Part1_GroundTruth/%s_segmentation.png" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class ISICGTDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
        self.root = root
        self.list_path = list_path
        ...
        self.files = []
        for name in self.img_ids:
            img_file = osp.join(self.root, 'train',"ISIC-2017_Training_Data/%s.jpg" % name)
            label_file = osp.join(self.root, 'train',"ISIC-2017_Training_Part1_GroundTruth/%s_segmentation.png" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class ISICDataTestSet(data.Dataset):
...
if __name__ == '__main__':
    dst = ISICDataSet("./data", is_transform=True)
